/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_split.c                                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: aes-salm <aes-salm@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/22 20:21:17 by aes-salm          #+#    #+#             */
/*   Updated: 2021/06/24 16:33:18 by aes-salm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/pipex.h"

int	get_words(char *s, char c)
{
	int	i;
	int	words;

	i = 0;
	words = 0;
	while (s[i])
	{
		if (s[i] != c && (s[i + 1] == c || s[i + 1] == '\0'))
			words++;
		i++;
	}
	return (words);
}

int	get_chars(char *s, char c)
{
	int	i;

	i = 0;
	while (s[i] != c && s[i])
		i++;
	return (i);
}

char	**free_all(char **out, int x)
{
	int	i;

	i = 0;
	while (i < x)
		free(out[i++]);
	free(out);
	return (NULL);
}

char	**fill_out(char *s, char **out, char c)
{
	int	i;
	int	x;
	int	xx;

	i = 0;
	x = 0;
	xx = 0;
	while (s[i] && x < get_words(s, c))
	{
		while (s[i] == c)
			i++;
		if (s[i] != c)
		{
			out[x] = malloc(sizeof(char) * get_chars(&s[i], c) + 1);
			if (!(out[x]))
				return (free_all(out, x));
		}
		while (s[i] != c && s[i])
			out[x][xx++] = s[i++];
		out[x++][xx] = '\0';
		xx = 0;
	}
	out[x] = NULL;
	return (out);
}

char	**ft_split(const char *ss, char c)
{
	char	**out;
	char	*s;

	s = (char *)ss;
	if (!s || *s == '\0')
	{
		out = malloc(sizeof(char *));
		*out = NULL;
	}
	else
	{
		out = malloc((get_words(s, c) + 1) * sizeof(char *));
		if (!(out))
			return (NULL);
		out = fill_out(s, out, c);
	}
	return (out);
}
